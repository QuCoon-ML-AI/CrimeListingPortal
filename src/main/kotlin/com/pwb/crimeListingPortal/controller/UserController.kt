...
class UserController(
	...
	@GetMapping("/read-by-user-role/{userRole}")
	fun readByUserRole(@RequestHeader("Authorization") authorization:String, @PathVariable userRole:String, @PathVariable stage: String):UserReadListResponse{
		environment.userInfo = environment.jwtUtil.privilegeAuthorization<LoginResponse>(
			ModulePrivilege(this::class.java.name.split(".").last().removeSuffix("Controller").camelToSnakeCase().uppercase(), PrivilegeConstant.READ),
			authorization)
		return userService.readByUserRole(userRole)
	}
